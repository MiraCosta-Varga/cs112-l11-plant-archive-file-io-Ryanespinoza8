public class Plant {
	...
	public Plant(String csv) {
		...
		try {
			...
			throw new IllegalArgumentException("CSV string doesn't have vaild temp (double) as second value in csv string. second vlaue = " + parts[1]);
		}
		String uses = parts[2];
		if (!this.setAll(name, temp, uses)) {
			throw new IllegalArgumentException("Invaild data provided after parsing, name=" + temp + ", uses =" + uses);
		}
